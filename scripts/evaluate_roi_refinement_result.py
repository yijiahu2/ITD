...
def run_merged_evaluation(
    ...
    roi_refinement_summary_path: str,
):
    base_cfg = load_yaml(base_config_path)
    summary = load_json(roi_refinement_summary_path)

    merged_shp = summary["merged_shp"]
    roi_root = str(Path(roi_refinement_summary_path).parent)

    merged_metrics_json = str(Path(roi_root) / "merged_metrics.json")
    merged_details_csv = str(Path(roi_root) / "merged_details.csv")

    cmd = [
        "python",
        "/home/xth/forest_agent_project/scripts/evaluate_reference_quality.py",
        "--inst_shp", merged_shp,
    ...
    compare_json = str(Path(roi_root) / "roi_refinement_compare_summary.json")
    save_json(compare, compare_json)

    print(f"[evaluate_roi_refinement_result] compare summary saved to: {compare_json}")
    return {
        "merged_metrics_json": merged_metrics_json,
        "merged_details_csv": merged_details_csv,
...
def main():
    ...
    parser.add_argument("--roi_refinement_summary", required=True)
    args = parser.parse_args()

    run_merged_evaluation(
        base_config_path=args.base_config,
        roi_refinement_summary_path=args.roi_refinement_summary,
    )
