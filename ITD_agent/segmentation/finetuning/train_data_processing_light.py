...
def run_tiny_backend(cfg: dict) -> Path:
    ...
    best_val = float("inf")
    best_ckpt = out_dir / "best_data_processing_light.pt"
    history = []
...
def _ensure_external_dataset(args_config: str, cfg: dict) -> Path:
    ...
    dataset_dir = output_dir / cfg.get("external_dataset_dirname", "external_data_processing_dataset")
    force_rebuild = bool(cfg.get("external_dataset_force_rebuild", False))
    dataset_source = str(cfg.get("external_dataset_source", "pseudo")).strip().lower()
    ...
    prepare_module = "ITD_agent.segmentation.finetuning.prepare_data_processing_external_dataset"
    if dataset_source == "public_coco":
        prepare_module = "ITD_agent.segmentation.finetuning.prepare_public_coco_data_processing_dataset"
    ...
    print(f"[RUN external dataset prepare] source={dataset_source}")
    print(" ".join(cmd))
    result = subprocess.run(cmd)
    if result.returncode != 0:
        raise RuntimeError(f"{prepare_module} failed")
...
def run_external_backend(args_config: str, cfg: dict) -> Path:
    ...
    if result.returncode != 0 and ckpt is None:
        raise RuntimeError("train_data_processing_light external backend failed")
