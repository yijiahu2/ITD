...
from ITD_agent.planning.agent.config_builder import load_yaml, save_yaml  # noqa: E402
...
def _default_generated_dir() -> Path:
    return Path(
        os.getenv(
            "FOREST_AGENT_PLANNING_CONFIG_DIR",
            str(_default_output_root() / "planning_scheduler" / "generated_configs"),
        )
    )
...
DEFAULT_RUNNER = "/home/xth/forest_agent_project/scripts/run_ITD_agent_experiment.py"
...
def try_import_prompt_builder():
    ...
    兼容不同 prompts.py 函数名
    """
    import ITD_agent.planning.agent.prompts as prompts_mod
...
def try_import_detail_extractor():
    ...
    """
    import ITD_agent.planning.agent.detail_tools as detail_mod
...
def try_call_doubao_json(prompt: str) -> Dict[str, Any]:
    ...
    """
    import ITD_agent.planning.agent.doubao_client as dc
