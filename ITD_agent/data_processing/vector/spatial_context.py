...
def summarize_reference_unit_terrain_classes(gdf: gpd.GeoDataFrame) -> Dict[str, Any]:
    if gdf is None or len(gdf) == 0:
        return {}

    summary: Dict[str, Any] = {
        "num_reference_units": int(len(gdf)),
...
def summarize_xiaoban_terrain_classes(gdf: gpd.GeoDataFrame) -> Dict[str, Any]:
    return summarize_reference_unit_terrain_classes(gdf)
...
def enrich_reference_clip_fields(
    clipped_gdf: gpd.GeoDataFrame,
    source_gdf: gpd.GeoDataFrame,
    ...
    return enrich_xiaoban_clip_fields(
        ...
        terrain_rule_cfg=terrain_rule_cfg,
    )
...
def clip_reference_vector_to_geometry(
    ...
    return clip_xiaoban_to_geometry(
...
def prepare_spatial_context(
    ...
    result = {
        ...
        "dom_terrain_context": {},
        "reference_vector_path": None,
        "inventory_vector_path": None,
        "reference_terrain_class_summary": {},
        "xiaoban_shp": None,
        "xiaoban_vector": None,
    ...
    if xiaoban_shp:
        ...
        result["reference_vector_path"] = str(xiaoban_clip)
        result["inventory_vector_path"] = str(xiaoban_clip)
        result["xiaoban_shp"] = str(xiaoban_clip)
        result["xiaoban_vector"] = str(xiaoban_clip)
        try:
            xiaoban_context = gpd.read_file(xiaoban_clip)
            result["reference_terrain_class_summary"] = summarize_reference_unit_terrain_classes(xiaoban_context)
            result["xiaoban_terrain_class_summary"] = summarize_xiaoban_terrain_classes(xiaoban_context)
        except Exception:
            result["reference_terrain_class_summary"] = {}
            result["xiaoban_terrain_class_summary"] = {}

    result["terrain_class_summary"] = result.get("reference_terrain_class_summary") or result.get("xiaoban_terrain_class_summary") or {}
