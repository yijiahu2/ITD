...
from ITD_agent.segmentation.finetuning.io_utils import dump_json, load_csv, load_yaml
...
def main() -> None:
    ...
    if len(df) == 0:
        raise RuntimeError("No usable samples with masks for external data processing training.")

    df = _ensure_split(df)
    tile_size = int(cfg.get("external_dataset_tile_size", 1024))
    tile_overlap = int(cfg.get("external_dataset_tile_overlap", 256))

    out_dir = Path(cfg["output_dir"]) / "external_data_processing_dataset"
    ...
    print(
        f"[OK] TCD external data processing dataset prepared: {out_dir}, "
