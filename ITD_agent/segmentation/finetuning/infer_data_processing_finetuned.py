...
from ITD_agent.segmentation.finetuning.io_utils import dump_json, dump_yaml, load_yaml, to_bool
...
def main() -> None:
    ...
    if not base_cfg.get("semantic_prior_script"):
        summary["reason"] = "base_config 未提供 semantic_prior_script。"
        dump_json(summary, out_dir / "integration_summary.json")
        print("[SKIP] missing semantic_prior_script.")
        return
    ...
    # 只保留一个权重入口
    new_cfg["semantic_prior_ckpt"] = str(ckpt_path.resolve())

    # 清理旧的 ckpt 参数，避免重复注入
    new_cfg["semantic_prior_extra_args"] = _strip_ckpt_args(new_cfg.get("semantic_prior_extra_args", []))
