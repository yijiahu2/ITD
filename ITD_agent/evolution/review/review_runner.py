...
from ITD_agent.evolution.review.asset_writers import (
...
from ITD_agent.evolution.review.io_utils import load_structured, write_csv, write_json, write_jsonl
from ITD_agent.evolution.review.review_context_builder import ReviewContext, build_llm_review_context, build_review_context
from ITD_agent.evolution.review.review_guardrails import V2WriteAction, assert_v2_guardrails, check_write_action
from ITD_agent.evolution.review.review_report_builder import build_review_report
from ITD_agent.evolution.review.reviewers import DistillationReviewer, FinetuneReviewer, MemoryReviewer, RoutingReviewer
from ITD_agent.evolution.review.skill_reviewer import build_class_level_skill_records
from ITD_agent.evolution.review.trajectory_compressor import compress_trajectory_for_review
from ITD_agent.evolution.review.trajectory_integrity_validator import validate_trajectory_integrity
from ITD_agent.evolution.review.trajectory_reader import list_v1_trajectories, read_trajectory
from ITD_agent.evolution.state.db import connect_state_db


def run_review_v2(config_path: str) -> dict[str, Any]:
    ...
    output_dir = Path(str(output_cfg.get("output_dir") or artifact_root / "v2_review"))
    ...
    with connect_state_db(db_path) as conn:
        ...
        for record in build_class_level_skill_records(review_run_id=review_run_id, source_run_id=source_run_id, contexts=contexts, cfg=cfg):
            guardrail = check_write_action(V2WriteAction.WRITE_SKILL_DRAFT, cfg)
            decision = {
...
def _guardrail_for_decision(decision: dict[str, Any], cfg: dict[str, Any]):
    target = decision.get("target_asset_type")
    action_by_target = {
        "memory": V2WriteAction.WRITE_MEMORY,
        "skill_draft": V2WriteAction.WRITE_SKILL_DRAFT,
        "finetune_sample": V2WriteAction.WRITE_FINETUNE_SAMPLE,
        "routing_candidate": V2WriteAction.UPDATE_ROUTING_POLICY if bool((cfg.get("routing_review") or {}).get("allow_routing_policy_update", False)) else V2WriteAction.WRITE_MEMORY,
        "distillation_candidate": V2WriteAction.START_DISTILLATION_JOB if bool((cfg.get("distillation_review") or {}).get("allow_distillation_job", False)) else V2WriteAction.WRITE_MEMORY,
    }
    return check_write_action(action_by_target.get(target, V2WriteAction.WRITE_MEMORY), cfg)
...
def _record_guardrail_block_checks(*, conn: sqlite3.Connection, review_run_id: str, source_run_id: str, cfg: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for action in [
        V2WriteAction.START_TRAINING_JOB,
        V2WriteAction.UPDATE_MODEL_WEIGHT,
        V2WriteAction.PROMOTE_MODEL,
        V2WriteAction.UPDATE_ROUTING_POLICY,
        V2WriteAction.WRITE_SKILL_ACTIVE_POLICY,
        V2WriteAction.START_DISTILLATION_JOB,
    ]:
