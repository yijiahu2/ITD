...
from ITD_agent.data_processing.remote_sensing.common import normalize_to_uint8, rgb_to_gray_uint8
...
def estimate_quality_from_rgb(rgb_u8: np.ndarray, valid_mask: np.ndarray) -> dict[str, Any]:
    quality = estimate_quality(rgb_u8, valid_mask)
    lap_var = float(quality.get("laplacian_variance") or 0.0)
    tenengrad = float(quality.get("tenengrad") or 0.0)
    if lap_var < 40.0 or tenengrad < 300.0:
        blur_score = 0.80
    elif lap_var < 100.0 or tenengrad < 700.0:
        blur_score = 0.55
    elif lap_var < 220.0 or tenengrad < 1600.0:
        blur_score = 0.30
    else:
        blur_score = 0.10
